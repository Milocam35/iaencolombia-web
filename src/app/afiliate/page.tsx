...
export const metadata: Metadata = {
  ...
  description:
    "Elige tu plan de afiliación a ACIA y continúa al pago en línea. Comunidad / Freemium y Gran empresa se gestionan por solicitud.",
};
...
export default async function AffiliationPage({
  ...
  const steps = [
    ["01", "Elige tu tipo de vinculación y plan"],
    ["02", "Completa tus datos de contacto"],
    ["03", "Continúa al pago o envía la solicitud de tu plan"],
  ];
      ...
      <main className="relative min-h-screen overflow-hidden bg-surface pt-28 pb-20">
        ...
        <div className="relative mx-auto grid w-full max-w-screen-xl grid-cols-[minmax(0,1fr)] gap-10 px-6 lg:grid-cols-[minmax(0,0.72fr)_minmax(0,1.28fr)] lg:items-start lg:gap-16">
          <section className="min-w-0 lg:sticky lg:top-32">
            ...
            <p className="mt-10 font-mono text-xs font-bold tracking-[0.22em] text-primary/60 uppercase">
              Vinculación ACIA
            </p>
            ...
            <p className="mt-6 max-w-xl text-lg leading-8 text-muted-foreground">
              {BOLD_MEMBERSHIP_PAYMENTS_ENABLED
                ? "Elige tu plan y completa tus datos para pagar en línea. Comunidad / Freemium y Gran empresa se gestionan por solicitud."
                : "Elige tu tipo de vinculación y un plan para conocer el valor y cómo continuar tu afiliación."}
            </p>
            {invalidPlan && (
              <p
                ...
                role="alert"
              >
                El plan indicado no existe. Selecciona una opción válida en el
                formulario.
              </p>
            )}
            <div className="mt-10 grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
              {steps.map(([number, text]) => (
                <div
                  key={number}
                  className="flex items-center gap-4 rounded-xl border border-primary/10 bg-white/70 p-4 backdrop-blur"
                >
            ...
            <p className="mt-8 text-xs leading-5 text-muted-foreground">
              Para los planes de pago, completa el pago en Bold. ACIA continuará
              el proceso de activación una vez confirmado.
            </p>
          </section>
